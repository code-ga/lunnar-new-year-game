...
import { Sparkles, Library, Gamepad2, User, Repeat, Coins, Cloud } from 'lucide-react';
...
const MainLayout: React.FC = () => {
    ...
    const navItems = [
        ...
        { id: 'admin', label: 'Admin', icon: User, path: '/admin' }, // Visible for demo, should be role protected
    ];
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen w-full bg-slate-50 overflow-hidden">
                    ...
                    {navItems.map((item) => (
                        <Link 
                            ...
                            className={`flex items-center space-x-3 w-full px-6 py-4 text-left transition-colors ${
                                location.pathname === item.path ? 'bg-indigo-50 text-indigo-600 border-r-4 border-indigo-600' : 'text-slate-500 hover:bg-slate-50'
                            }`}
                        >
                            <item.icon size={20} className={location.pathname === item.path ? 'fill-indigo-600' : ''} />
            ...
            <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-200 flex justify-around z-30 pb-safe">
                {navItems.filter(i => i.id !== 'exchange').map((item) => (
                    <Link 
                        key={item.id} 
                        to={item.path}
                    ...
                    >
                        <item.icon size={24} className={location.pathname === item.path ? 'fill-indigo-600' : ''} />
