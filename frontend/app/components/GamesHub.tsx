...
const GamesHub: React.FC<GamesHubProps> = ({ activeGame, setActiveGame }) => {
	if (activeGame) {
		return (
			<div className="h-full relative bg-slate-900 overflow-hidden">
				...
				{activeGame === "race" && <TurboClickGame />}
				{activeGame === "taixiu" && <TaiXiuGame />}
				{activeGame === "baucua" && <BauCuaGame />}
			</div>
		);
	}
		...
		<div className="p-6 h-full flex flex-col items-center">
			<div className="text-center mb-8">
				...
				<p className="text-slate-500 font-medium">
					Chơi các mini-game để kiếm thêm Xu gacha!
				</p>
			</div>
			...
			<div className="grid grid-cols-2 gap-4 w-full max-w-2xl pb-20">
				{games.map((game) => (
					...
					>
						...
						<span className="font-black text-lg leading-tight uppercase tracking-tighter">
							{game.label}
						</span>
			...
			<div className="mt-auto mb-10 p-4 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-start gap-3 max-w-sm">
				...
				<p className="text-indigo-900 text-xs font-medium leading-relaxed">
					Mẹo: <span className="font-bold">Turbo Click</span> là cách nhanh nhất
					để kiếm xu nếu bạn có đôi tay linh hoạt!
				</p>
			</div>
		</div>
	);
