    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-full p-6 animate-pop-in">
        <div className="mb-8 relative w-48 h-72">
            ...
            <div className={`relative w-full h-full rounded-2xl shadow-2xl border-4 ${config.border} flex flex-col items-center justify-between p-4 bg-white overflow-hidden`}>
                <div className={`absolute inset-0 ${config.color} opacity-20 ${isEx ? 'ex-gradient animate-spin-slow' : ''}`}></div>
                <div className="z-10 font-bold text-center w-full text-slate-700 text-lg px-1 leading-tight">{result.name}</div>
                <div className={`z-10 w-24 h-24 rounded-full shadow-inner ${config.color} flex items-center justify-center text-white font-black text-3xl ${isEx ? 'ex-gradient' : ''}`}>
                    {result.rarity}
                </div>
                <div className="z-10 w-full flex flex-col items-center">
                    <span className={`px-4 py-1.5 rounded-full text-white font-bold text-sm ${config.color} ${isEx ? 'ex-gradient' : ''}`}>
                      {result.rarity} Rank
                    </span>
                    <span className="text-xs text-slate-400 mt-2 font-mono">#ID:{result.uniqueId}</span>
                </div>
                <div className="absolute top-0 right-0 bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded-bl-lg z-20 animate-pulse">MỚI</div>
            </div>
        </div>

        <h2 className="text-3xl font-black text-indigo-900 mb-2">{result.name}</h2>
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-full space-y-8 p-4">
      <div className="text-center space-y-2">
          ...
          <p className="text-indigo-600 font-medium">Lấy ngay một gối ôm ngẫu nhiên chỉ với {GACHA_COST} Xu</p>
      </div>
      ...
      <div className="relative group">
        ...
        >
            ...
            <div className="mt-2 text-indigo-400 text-xs font-bold uppercase tracking-widest">Mystery Box</div>
        </button>
      </div>

      <div className="bg-white/50 backdrop-blur-sm p-3 rounded-2xl border border-indigo-50 flex items-center gap-2">
        <Sparkles size={16} className="text-indigo-400" />
        <span className="text-slate-500 text-sm font-medium">Hệ thống gacha công bằng - Tỉ lệ EX 0.1%</span>
      </div>
    </div>
  );
};
