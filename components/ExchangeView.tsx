...
import { PILLOW_TEMPLATES } from '../constants';
...
const ExchangeView: React.FC<ExchangeViewProps> = ({ addToInventory }) => {
  ...
  const handleImport = () => {
    ...
    const template = PILLOW_TEMPLATES.find(t => t.id === templateId);
    ...
    if (template) {
      ...
      setStatus({ type: 'success', msg: `Thành công! Bạn nhận được: ${template.name}` });
      setCode('');
    } else {
  ...
  return (
    <div className="p-6 h-full flex flex-col items-center">
      <div className="text-center mb-10">
          <h2 className="text-3xl font-black text-slate-800 tracking-tight">Nhập Mã Quà Tặng</h2>
          <p className="text-slate-500 font-medium">Nhập Pillow Code để nhận thêm gối mới vào túi đồ.</p>
      </div>
      ...
      <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-xl border border-slate-100 space-y-6">
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Vui lòng nhập code</label>
          <input 
            type="text" 
            value={code}
        ...
        {status && (
          <div className={`p-4 rounded-2xl flex items-start gap-3 animate-pop-in ${status.type === 'success' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'}`}>
      ...
      <div className="mt-12 p-6 bg-slate-100 rounded-3xl border border-slate-200 w-full max-w-md">
          <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3">Thông tin mã code</h4>
