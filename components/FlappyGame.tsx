  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (frameRef.current % 100 === 0) {
        ...
        pipes.current.push({ x: canvas.width, top: topHeight, bottom: canvas.height - topHeight - gap });
      }
      ...
      pipes.current.forEach(p => {
        if (p.x < 70 && p.x > 30) {
            if (bird.current.y < p.top || bird.current.y > canvas.height - p.bottom) {
                finishGame(score);
            }
        }
    ...
    <div className="flex flex-col items-center justify-center p-4 space-y-6 h-full min-h-[450px]">
      {gameState === 'idle' || gameState === 'loading' ? (
        <div className="text-center space-y-6 animate-in fade-in zoom-in duration-500">
           ...
           <div>
            <h3 className="text-2xl font-black text-slate-800">Flappy Pillow</h3>
            <p className="text-slate-500 font-medium">Bay qua chướng ngại vật để nhận xu!</p>
           </div>
           <button 
             onClick={startGame} 
        ...
        <div className="relative group cursor-pointer select-none touch-none" onMouseDown={handleAction} onTouchStart={handleAction}>
           <canvas ref={canvasRef} width={320} height={400} className="bg-white rounded-3xl shadow-2xl border-4 border-slate-100" />
        ...
        <div className="text-center space-y-8 animate-pop-in">
           <div className="relative">
             ...
             <div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-indigo-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg">KẾT QUẢ</div>
           </div>
           <div>
             <div className="text-6xl font-black text-indigo-900 mb-1">{score}</div>
             <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Điểm đạt được</p>
           </div>
