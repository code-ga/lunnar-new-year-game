        ...
        try {
            ...
            if (res.ok) {
                ...
                setTimeout(() => {
                    ...
                    setMessage(win ? `🎉 BẠN THẮNG ${data.winAmount} XU!` : '😢 Chúc bạn may mắn lần sau');
                }, 1000);
            } else {
                setIsRolling(false);
    ...
    return (
        <div className="flex flex-col items-center p-6 space-y-8 animate-in zoom-in duration-300">
            ...
            <div className="flex gap-4">
                {dice.map((d, i) => (
                    <div key={i} className={`w-20 h-20 bg-white rounded-2xl shadow-xl flex items-center justify-center text-4xl font-black border-4 border-slate-100 ${isRolling ? 'animate-bounce' : ''}`}>
                        {d}
                    </div>
                ))}
            </div>
            ...
            <div className="text-center">
                <div className="text-2xl font-black text-indigo-900 mb-2">{message}</div>
