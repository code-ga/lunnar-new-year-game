...
const BauCuaGame: React.FC = () => {
    ...
    const placeBet = (idx: number) => {
        const cost = 10;
        if (!user || (user.coins ?? 0) < totalBetSize + cost) return alert("Không đủ xu!");
        const newBets = [...bets];
        newBets[idx] += cost;
        setBets(newBets);
    };
    ...
    const play = async () => {
        ...
        setIsRolling(true);
        try {
            const res = await fetch('/api/game/play/probability', {
                method: 'POST',
        ...
        <div className="flex flex-col items-center p-4 space-y-6">
            <div className="grid grid-cols-3 gap-3 w-full max-w-sm">
                {SYMBOLS.map((s, i) => (
                    ...
                    >
                        <span className="text-4xl group-hover:scale-110 transition-transform">{s}</span>
                        {bets[i] > 0 && (
            ...
            <div className="flex gap-4 bg-slate-100 p-4 rounded-3xl shadow-inner w-full max-w-xs justify-center">
                {result.map((r, i) => (
                    <div key={i} className={`w-16 h-16 bg-white rounded-2xl shadow-lg border-2 border-indigo-50 flex items-center justify-center text-3xl ${isRolling ? 'animate-bounce' : ''}`}>
                        {SYMBOLS[r]}
                    </div>
                ))}
            </div>
            ...
            <div className="flex gap-3 w-full max-w-sm">
                <button onClick={clearBets} className="flex-1 py-3 bg-slate-200 text-slate-600 font-bold rounded-xl active:scale-95 transition-transform">XÓA CƯỢC</button>
                <button 
                    onClick={play} 
            ...
            {lastWin > 0 && (
                <div className="text-center animate-bounce">
                    <div className="text-2xl font-black text-yellow-500">🎉 +{lastWin} XU 🎉</div>
                </div>
            )}
        </div>
    );
